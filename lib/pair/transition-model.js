...
const RejectReason = Object.freeze({
  ...
  ACTUAL_PATHS_INVALID: 'ActualPathsInvalid',
  ACTUAL_PATH_OUTSIDE_BOUNDARY: 'ActualPathOutsideBoundary',
  FORBIDDEN_PATH: 'ForbiddenPath',
  INTEGRATED_OWNERSHIP_OVERLAP: 'IntegratedOwnershipOverlap',
...
function normalizedRepoPath(rawPath) {
  ...
  let value = rawPath.normalize('NFC').replace(/\\/g, '/')
...
const FORBIDDEN_CONTROL_FILES = new Set([
...
function forbiddenControlPath(repoPath) {
  const folded = repoPath.toLowerCase()
  return folded === '.git' || folded.startsWith('.git/') ||
    folded === '.baton-pass' || folded.startsWith('.baton-pass/') ||
    folded === 'refs/baton-pass' || folded.startsWith('refs/baton-pass/') ||
    FORBIDDEN_CONTROL_FILES.has(folded)
}

function canonicalPathEntry(entry) {
  if (!isRecord(entry) || !['file', 'tree'].includes(entry.kind)) return null
  let value = normalizedRepoPath(entry.path)
  if (!value || forbiddenControlPath(value)) return null
...
function pathWithinBoundary(repoPath, boundary) {
  return boundary.some(entry => entry.kind === 'file' ? repoPath === entry.path : repoPath.startsWith(entry.path))
}

function candidatePathCheck(state, itemId, claim, rawPaths) {
  if (!Array.isArray(rawPaths)) return reject(RejectReason.ACTUAL_PATHS_INVALID)
  const normalized = []
  const identities = new Set()
  for (const rawPath of rawPaths) {
    const repoPath = normalizedRepoPath(rawPath)
    if (!repoPath) return reject(RejectReason.ACTUAL_PATHS_INVALID, rawPath)
    if (forbiddenControlPath(repoPath)) return reject(RejectReason.FORBIDDEN_PATH, repoPath)
    const folded = repoPath.toLowerCase()
    if (identities.has(folded)) return reject(RejectReason.ACTUAL_PATHS_INVALID, repoPath)
    identities.add(folded)
    if (!pathWithinBoundary(repoPath, claim.effectiveBoundary)) {
      return reject(RejectReason.ACTUAL_PATH_OUTSIDE_BOUNDARY, repoPath)
    }
    normalized.push(repoPath)
  }
  for (const [otherId, otherItem] of Object.entries(state.items)) {
    if (otherId === itemId || otherItem.status !== 'integrated' || itemsOrdered(state.plan.items, itemId, otherId)) continue
    const ownedPaths = otherItem.integrated && otherItem.integrated.actualPaths
    if (Array.isArray(ownedPaths) && normalized.some(repoPath => ownedPaths.includes(repoPath))) {
      return reject(RejectReason.INTEGRATED_OWNERSHIP_OVERLAP, otherId)
    }
  }
  normalized.sort()
...
function planItemsFrom(configItems) {
  ...
  for (const raw of configItems) {
    if (!isRecord(raw) || !nonempty(raw.id) || hasOwn(items, raw.id)) return null
...
function createInitialState(config) {
  try {
    ...
    return success({
      ...
      machineLabels: Object.create(null),
      nextOrdinals: Object.create(null),
      items,
      claims: Object.create(null),
      contractIncidents: Object.create(null),
      integrationGate: null,
      planGate: null,
      attempts: Object.create(null),
      candidateRefs: Object.create(null),
      eventIds: Object.create(null),
      controlOids: Object.assign(Object.create(null), { [config.controlOid]: true }),
    })
  } catch (error) {
...
function verifyCommonFence(state, event, facts) {
  ...
  if (hasOwn(state.eventIds, event.eventId)) return reject(RejectReason.DUPLICATE_EVENT)
  ...
  if (hasOwn(state.controlOids, facts.nextControlOid)) return reject(RejectReason.CONTROL_OID_REUSED)
...
function applyRegister(state, event, facts) {
  ...
  if (hasOwn(state.registrations, event.registrationId)) return reject(RejectReason.INVALID_EVENT, 'registration id reused')
...
function applyClaim(state, event, facts) {
  ...
  if (!nonempty(event.claimId) || hasOwn(state.claims, event.claimId)) return reject(RejectReason.CLAIM_ID_REUSED)
...
function applyContractChanged(state, event, facts) {
  ...
  if (!owned.ok) return owned
  if (!nonempty(event.incidentId) || hasOwn(state.contractIncidents, event.incidentId)) return reject(RejectReason.INCIDENT_EXISTS)
...
function applyDone(state, event, facts) {
  ...
  if (!nonempty(event.candidateRef) || hasOwn(state.candidateRefs, event.candidateRef)) return reject(RejectReason.CANDIDATE_COLLISION)
  ...
  const checkedPaths = candidatePathCheck(state, event.itemId, owned.claim, event.actualPaths)
  ...
  const record = {
    ...
    actualPaths: checkedPaths.paths,
...
function applyIntegrationStarted(state, event, facts) {
  ...
  if (hasOpenIncident(state, event.itemId)) return reject(RejectReason.INCIDENT_STILL_OPEN)
  if (!nonempty(event.integrationAttemptId) || hasOwn(state.attempts, event.integrationAttemptId)) return reject(RejectReason.INTEGRATION_ATTEMPT_REUSED)
...
function exactIntegrationEvidence(state, event, facts, gate, candidate) {
  ...
  return facts.integrationRefOid === event.mergeOid && isRecord(merge) && isRecord(report) &&
    ...
    report.pathDigest === candidate.manifestDigest
}
...
function applyIntegrated(state, event, facts) {
  ...
  if (item.status === 'integrated') {
    ...
    if (identitiesEqual(integrationIdentity(recorded), integrationIdentity(event))) return reject(RejectReason.ALREADY_INTEGRATED)
  ...
  if (!matchingGate(state, event, owned)) return reject(RejectReason.INTEGRATION_EVIDENCE_MISMATCH)
  const gate = state.integrationGate
  const candidate = owned.claim.candidates.find(entry => entry.ref === gate.candidateRef && entry.generation === gate.candidateGeneration)
  if (!candidate) return reject(RejectReason.CANDIDATE_NOT_PUBLISHED)
  const checkedPaths = candidatePathCheck(state, event.itemId, owned.claim, candidate.actualPaths)
  if (!checkedPaths.ok) return checkedPaths
  if (!exactIntegrationEvidence(state, event, facts, gate, candidate)) return reject(RejectReason.INTEGRATION_EVIDENCE_MISMATCH)
  const identity = integrationIdentity(event)
  owned.claim.status = 'integrated'
  owned.item.status = 'integrated'
  owned.item.activeClaimId = null
  owned.item.integrated = { ...identity, actualPaths: checkedPaths.paths }
